 ...
 */
THREE.DragRotateControls = function(_camera, _objects, _domElement, _alpha) {

    var drc = this;
    this.domElement = ( _domElement !== undefined ) ? _domElement : renderer.domElement;
    ...
    // 旋转方法
    var alpha = ( _alpha !== undefined ) ? 1 : -1;
    //API
    this.noRotate = false;
    this.noZoom = false;
    this.noPan = false;
    ...
    this.rotateModel = (function () {
        ...
        return function rotateModel() {
            delta.set(alpha * _moveCurr.x - alpha * _movePrev.x, alpha * _moveCurr.y - alpha * _movePrev.y, 0);
            angle = delta.length();
            if (angle > 0) {
                axis.crossVectors(projectOnTrackball(alpha * _movePrev.x, -alpha * _movePrev.y), projectOnTrackball(alpha * _moveCurr.x, -alpha * _moveCurr.y)).normalize();
                angle *= singleObj.rotateSpeed * 10;
                iftouchRotate = false;
                lastAngle = angle;
                //console.log(axis);
            }
        ...
        return function panModel() {
            if (_state === STATE.PAN) {
                ...
                if (panDelta.length() != 0) {
                    panDelta.multiplyScalar(singleObj.moveSpeed * objScele * 0.7);
                ...
                if (panBoo) {
                    ...
                    if (panDelta.length() != 0) {
                        _zoomBoolen = false;
                        panDelta.multiplyScalar(singleObj.moveSpeed * objScele * 0.5);
                        _objects.position.add(panDelta);
                    }
                }
            }
    ...
    this.zoomModel = (function () {
        ...
        return function zoomModel() {
            fovDiffer = (singleObj.maxfov - singleObj.minfov) / 5;
            stalls = fovDiffer / 30;
            if (_state === STATE.ZOOM) {
                ...
                if (_wheelNint < 10) {
                    _wheelNum = delta * stalls * 2 * singleObj.zoomSpeed;
                }
                else if (_wheelNint >= 10 && _wheelNint < 30) {
                    _wheelNum = delta * stalls * singleObj.zoomSpeed;
                }
                ...
                _camera.zoom = Math.max(singleObj.minfov, Math.min(singleObj.maxfov, _camera.zoom - _wheelNum));
                ...
                if (_zoomBoolen) {
                    if (_wheelNint < 10) {
                        _wheelNum = delta * stalls * 2 * 2 * singleObj.zoomSpeed;
                    }
                    ...
                    _wheelNint++;
                    _camera.zoom = Math.max(singleObj.minfov, Math.min(singleObj.maxfov * 2, _camera.zoom - _wheelNum));
                }
            }
    ...
    this.doubleTouch = (function() {
        ...
        return function doubleTouch() {
            ...
            if (0 < touchTimeDev && touchTimeDev <= 400 && touchDelta.length() <= 50 && iftouchRotate) {
                ...
                zoomBoo = true;
                if (_camera.zoom >= singleObj.maxfov * 2) {
                    deltas = 3;
                }
                else if(_camera.zoom <= singleObj.minfov){
                    deltas = -3;
                }
                touchTimeDev = -1;
            }
            ...
            if (zoomBoo) {
                if (wheelnint < 10) {
                    wheelnum = deltas * stalls * singleObj.zoomSpeed;
                }
                ...
                wheelnint++;
                _camera.zoom = Math.max(singleObj.minfov, Math.min(singleObj.maxfov * 2, _camera.zoom - wheelnum));
            }
        }
    }());
